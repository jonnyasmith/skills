...
- Output path: `docs/<feature_name>/discovery.md`
...
Write a Markdown file to `docs/<feature_name>/discovery.md`. Create the `docs/<feature_name>/` directory if it doesn't exist. See [MARKDOWN-REPORT.md](MARKDOWN-REPORT.md) for the document skeleton, the Mermaid diagram catalogue, and style guidance.

Markdown over HTML: the report must stay digestible for both a human reader and another LLM. No SVG coordinate noise, no CSS — just prose, tables, and Mermaid fenced blocks. The full range of Mermaid diagram types is available; pick the one that matches the shape of what you're describing rather than forcing everything into a flowchart.

Print the absolute path. Offer to open it, but the file is meant to be read as text.
...
When all High and Medium impact gaps are resolved, offer to re-render the Markdown report with resolved gaps struck through and new findings incorporated. Then stop.
...
- Output is Markdown + Mermaid only — no HTML, no SVG. Pick the Mermaid diagram type that fits the structure; the full catalogue is in [MARKDOWN-REPORT.md](MARKDOWN-REPORT.md).
