...
spread across roughly 25 repositories. Use the **AI Wiki** repository,
...
3. Accept a candidate only when it contains both `wiki/index.md` and `raw/repos/`.
   If no valid candidate is available, ask the user where the AI Wiki is checked out.
...
4. **Verify only the needed facts.** Read the narrow code, config, project, schema,
   ...
   missing, ask the user to populate the AI Wiki clones before answering.
...
- **Do not load repo instruction files as domain evidence.** `AGENTS.md`, `CLAUDE.md`,
  ...
  platform. Skip them unless the user explicitly asks about the AI Wiki maintenance
