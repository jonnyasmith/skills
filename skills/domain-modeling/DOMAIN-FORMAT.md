...
A context's `domain.md` is its glossary — nothing else. It lives at `docs/agents/domain.md` for the context you're in.
...
- **Defer solution-wide terms upward.** A working target's `domain.md` holds only its inner terms; terms shared across the whole solution live in the root `domain.md`. Don't restate them.
