...
- Keep the progress-page line in every prompt, and copy `PROGRESS.html` from this skill next to the work when you are the one running the loop. It is the shape that page should take, and its comments carry the rules for keeping it honest — one writer, never green ahead of the evidence, and it says which half of the bar was actually reached.
